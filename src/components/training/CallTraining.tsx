...
import { Mic, Volume2, VolumeX, Phone, Star, Shield, Target } from "lucide-react";
...
function ModuleCard({ icon, title, description, difficulty, onClick }: ModuleCardProps) {
  return (
...
export default function CallTraining() {
  ...
  const scoreResponse = (response: string, scenario: TrainingScenario): number => {
    const scores = {
      tonality: 0,
      phrasing: 0,
      empathy: 0
    ...
    Object.entries(scenario.scoringCriteria).forEach(([category, criteria]) => {
      ...
      scores[category as keyof typeof scores] = Math.max(0, Math.min(100, categoryScore)) * 
        (criteria.weight / 100);
    });
  ...
  const handleResponse = useCallback(async (response: string) => {
    ...
    setModuleProgress(prev => ({
      ...
      averageScore: Math.round((prev.averageScore * prev.totalResponses + score) / 
    ...
    if (isSpeaking) {
      ...
      try {
        const cleanedText = cleanTextForSpeech(currentScenario.prospectResponse);
        await speakText(
          cleanedText,
    ...
    setCallState(prev => ({
      ...
      isComplete,
      feedback: isComplete ? "Great job completing the module!" : "Good response, waiting for prospect..."
    }));
  ...
  }, [currentScenario, currentModule, callState.currentStep, isSpeaking, sessionVoice]);
  ...
  const isSectionUnlocked = (section: TrainingSection): boolean => {
    if (!section.unlockCriteria) return true;
    
    const { sectionIds, minScore } = section.unlockCriteria;
    
    return sectionIds.every(id => 
      sectionProgress[id]?.completed && 
      sectionProgress[id]?.score >= minScore
    );
  };
    ...
    <div className="space-y-6">
      {!selectedModule ? (
        <div className="bg-gradient-to-br from-blue-900 to-blue-950 p-6 rounded-lg border border-blue-700">
          <h2 className="text-2xl font-bold text-white mb-4">Select a Training Module</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {TRAINING_SECTIONS.map((section) => (
              <SectionProgress
                ...
                isLocked={section.unlockCriteria && !isSectionUnlocked(section)}
                onModuleSelect={(moduleId) => {
                  if (section.unlockCriteria && !isSectionUnlocked(section)) {
                    return;
                  }
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-white">
                {TRAINING_SECTIONS.find(s => s.modules.some(m => m.id === selectedModule))?.title}
              </h2>
            </div>
            <Button 
              variant="outline" 
            ...
            <>
                  ...
                  {callState.currentStep > 0 && (
                    <div className="bg-blue-950/50 p-3 rounded">
                      ...
                      <p className="text-white">
                        {currentModule?.scenarios[callState.currentStep - 1].prospectResponse}
                      </p>
                    </div>
                  )}
                  <div>
                    ...
                    <p className="text-white text-lg">
                      {currentModule?.scenarios[callState.currentStep].agentScript}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-900/20 p-3 rounded border border-blue-800">
                ...
                <p className="text-blue-100">
                  {currentModule?.scenarios[callState.currentStep].expectedResponse}
                </p>
              </div>
            </>
          ) : null}
          ...
          {showLevelComplete && (
            <LevelComplete
              level={{
                ...
                name: TRAINING_SECTIONS.find(s => s.modules.some(m => m.id === selectedModule))?.title || '',
                description: TRAINING_SECTIONS.find(s => s.modules.some(m => m.id === selectedModule))?.description || '',
                difficulty: TRAINING_SECTIONS.find(s => s.modules.some(m => m.id === selectedModule))?.modules[0].difficulty || 'BEGINNER',
                minScore: 0,
                objectionTypes: [],
